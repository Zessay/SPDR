...
@Tokenizer.register("chinese_character")
class ChineseCharacterTokenizer(Tokenizer):
    def __init__(self,
    ...
    def tokenize(self, text: str) -> List[TokenAdd]:
        ...
        # tokenize the chinese char
        text = self._tokenize_chinese_chars(text)
        orig_tokens = whitespace_tokenize(text)
        split_tokens = []
        for token in orig_tokens:
            if self._do_lowercase and (token not in self._never_split):
                token = token.lower()
                token = self._run_strip_accents(token)
            split_tokens.extend(self._run_split_on_punc(token))

        output_tokens = whitespace_tokenize(" ".join(split_tokens))
        return [TokenAdd(t) for t in output_tokens]

    def _run_split_on_punc(self, text: str):
        """Splits punctuation on a piece of text."""
        if text in self._never_split:
            return [text]
        chars = list(text)
        i = 0
        start_new_word = True
