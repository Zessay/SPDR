...
from resolution.common.data.token_indexer.pretrained_chinese_bert_indexer \
    import PretrainedChineseBertIndexer


@TokenIndexer.register("pretrained_chinese_bert_mismatched")
class PretrainedChineseBertMismatchedIndexer(TokenIndexer):
    """
    Use this indexer when (for whatever reason) you are not using a corresponding
    `PretrainedChineseBertTokenizer` on your input. We assume that you used a tokenizer that splits
    ...
    `PretrainedChineseBertMismatchedIndexer` to embed these wordpieces and then pull out a single
    vector for each original word.

    Registered as a `TokenIndexer` with name "pretrained_chinese_bert_mismatched".
    ...
    max_length : `int`, optional (default = `None`)
        ...
        the same value as the `max_length` option on the `PretrainedChineseBertMismatchedIndexer`.
    """
    ...
    def __init__(self,
        ...
        self._matched_indexer = PretrainedChineseBertIndexer(model_name,
                                                             namespace,
                                                             max_length,
                                                             **kwargs)
    ...
    @overrides
    def count_vocab_items(self, token: TokenAdd,
                          counter: Dict[str, Dict[str, int]]):
        return self._matched_indexer.count_vocab_items(token, counter)

    @overrides
    def tokens_to_indices(self, tokens: List[TokenAdd],
                          vocabulary: Vocabulary) -> IndexedTokenList:
        self._matched_indexer._add_encoding_to_vocabulary_if_needed(vocabulary)

        # don't need to add special tokens
        ...
        output: IndexedTokenList = {
            ...
            "turn_ids": [t.turn_id for t in wordpieces]
        }
    ...
    def as_padded_tensor_dict(self,
        ...
        tensor_dict["offsets"] = torch.LongTensor(pad_sequence_to_length(offsets_tokens,
                                                                         offsets_padding_lengths,
                                                                         default_value=lambda: (0, 0)))
        return tensor_dict

    def __eq__(self, other):
        if isinstance(other, PretrainedChineseBertMismatchedIndexer):
